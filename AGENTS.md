...
| `translations.py` | **译名表数据（唯一权威）**：`ENTRIES`（标准名 + pattern/ja/en/cat/full_name/aliases/note；aliases 元素为 `V(写法, Source.X)` 枚举标注（Source：OFFICIAL_HANS 官简/OFFICIAL_HANT 官繁/FAN 民间），别名位于更长他名内部时用 `pre=`/`post=` 挂 lookaround guard；full_name=角色条目完整标题（全名或真名，称呼归到真名），仅供一次性移动与数据参考，不进替换链；`main=False` 不生成名字模糊规则；`record_only`=非归一目标的记录项（文档标注，无行为门控）；顺序即替换链顺序）+ `RECORD_ONLY`（特判不处理，永不生成名字规则）+ `SIMILAR_CHARS`。纯数据无逻辑，供 user-fixes import 生成替换表，也供 LLM 翻译管线与 re0-corpus 审查管线直接消费 |
