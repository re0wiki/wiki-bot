...
| `scripts/` | 顶层只放 pwb 任务脚本（`re0_*` ×7，见下行；pwb 按名在顶层解析）。`scripts/tools/`：非 pwb 的常驻/维护工具（直接 python 运行）：`recent_changes_watchdog.py`、诊断（`verify_wiki_access.py`/`test_pwb_throttle.py`）、审计（`dump_modules.py`/`template_inventory.py`/`template_complexity.py`/`recheck_template_usage.py`/`scan_title_prefixes.py`/`check_css_imports.py`/`audit_wikipedia_links.py`）。`scripts/oneoff/`：一次性脚本归档（含 429 探测 `probe_*`，重跑传完整路径）。`logs/` 是纯 scratch（gitignore）——分辨标准：常驻→scripts/ 或 tools/；一次性但 docs 引为出处或可能复跑→oneoff/（入 git）；纯探索中间产物→logs/，且 **docs 只写结论、不引用 logs/ 路径**（防新 clone 悬空引用） |
