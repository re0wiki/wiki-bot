...
| `translations.py` | **译名表数据（唯一权威）**：`ENTRIES`（标准名 + pattern/ja/en/cat/full_name/aliases/note；aliases 元素为 `V(写法, Source.X)` 枚举标注（Source：OFFICIAL_HANS 官简/OFFICIAL_HANT 官繁/FAN 民间），别名位于更长他名内部时用 `pre=`/`post=` 挂 lookaround guard；full_name=角色条目完整标题（全名或真名，称呼归到真名），仅供一次性移动与数据参考，不进替换链；`fuzzy=False` 不生成名字模糊规则（短名防误判/特判不处理，别名规则照常）；顺序即替换链顺序）+ `SIMILAR_CHARS`。纯数据无逻辑，供 user-fixes import 生成替换表，也供 LLM 翻译管线与 re0-corpus 审查管线直接消费 |
...
2. 改译名 = 改 `translations.py` 的 `ENTRIES`：`name` 进主列表（`p2o()` 自动生成别名正则），`aliases` 登记组外异写精确对（须带 Source 来源标注；别名位于更长他名内部时用 `pre=`/`post=` 挂 guard，如 裘斯 `pre="(?<!梅)"`；不同日文名（真名/旧名/称呼）不互转，各自以 fuzzy=False 单记），`fuzzy=False` 用于 2 字短名（**不做模糊匹配**，只登记已知别名——2 字 p2o 展开的命中几乎全是普通词，实测「贝蒂那里」被打成 贝缇娜）、两个字落在同一相似组（p2o 组合爆炸，实测命中 哈鲁特/亚基 等他名/普通词）、或首字展开会吃普通词（梅莉：`f('梅')` 含 美，会把 美丽 打成 梅莉）等防误判场景。覆盖新变体优先扩 `SIMILAR_CHARS` 相似组让 `p2o()` 自动生成（如 伊/易 组覆盖 路易→鲁伊）；选择性 f() 展开的别名规则（利格鲁/梅莉）与模板替换规则留在 `user-fixes.py` 的 `translation_manual` 内联段。拿不准相似字符覆盖面的，先 `python main.py fix:translation -s` 干跑。标题含别名的页面由 `re0_move` 任务用同一张表自动移动，无需另行处理。
