...
- 排除 bot：过滤 EXCLUDE_USERS（bot 账号）与带 bot 标记的编辑。
...
OVERLAP_SEC = 600  # 回看窗口重叠量，配合 rcid 去重兜底时钟/写入乱序
MAX_LOOKBACK_SEC = 72 * 3600  # 状态过旧时最多回看 72h（rclimit=500 也兜底）
UA = "re0wiki-rc-watchdog/1.0 (https://github.com/re0wiki/wiki-bot)"


def fetch_recent_changes(since_ts: float) -> list[dict]:
    params = {
        "action": "query",
        "list": "recentchanges",
        "rcstart": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "rcend": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(since_ts)),
...
def main() -> int:
    ...
    last_run = float(state.get("last_run", now))
    since = max(now - MAX_LOOKBACK_SEC, last_run - OVERLAP_SEC)
    ...
    if last_rcid == 0:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": UA})
        with urllib.request.urlopen(req, timeout=60) as resp:
            latest = json.load(resp)["query"]["recentchanges"]
        last_rcid = max((c["rcid"] for c in latest), default=0)

    changes = fetch_recent_changes(since)
