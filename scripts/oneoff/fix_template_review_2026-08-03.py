...
NL = r"\r?\n"
for t in ["Elf", "Seirei", "Yousei", "加护", "Tooltip", "Seirei or Elf", "Yousei or Elf"]:
    edits.append((
        f"Template:{t}",
...
edits.append((
    ...
    [(
        re.compile(
            re.escape("</noinclude>") + NL + re.escape("<noinclude>[[Category:格式模板]]</noinclude>")
        ),
...
for t in [
    "Disambiguation", "Elf", "Kana2Romaji", "QA list", "Ringa",
    "Ruby-ja", "Seirei or Elf", "Seirei", "T category",
]:
    edits.append((
        f"Template:{t}/doc",
...
edits.append((
    ...
    [
        (
            re.compile(re.escape("[http://www.twitter.com/{{{#|}}}/ {{{#|}}} / Twitter]")),
...
edits.append((
    ...
    [
        (
            re.compile(
                re.escape("'''参数名为 <code>#</code>（井号）'''，调用时必须写成 <code>|#=用户名</code>。")
            ),
