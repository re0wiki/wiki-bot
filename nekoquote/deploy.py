...
main_src = Path("logs/nekoquote/main.lua").read_text(encoding="utf-8")
...
changed = []
for f in sorted(Path("logs/nekoquote/lua").glob("*.lua")):
    live = Path("logs/nekoquote/lua_live") / f.name
...
for m in changed:
    content = (Path("logs/nekoquote/lua") / f"{m}.lua").read_text(encoding="utf-8")
