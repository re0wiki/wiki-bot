...
- **译名一致性**：LLM prompt **注入 ja→zh 专有名词表**（2026-08-09 用户裁决，反转当天早些时候的「不注入」决定）：目的不是规定用字，而是让弱模型**识别专有名词**、不把角色名当普通名词（试点 v1 曾把 パック 译成「包包」）。表由 `logs/p0_build_name_table.py` 从 zh 站角色页 infobox（name/name_ja_kanji/name_ja_kana，跟随重定向，短名取首段）抓取 + 手工补充段（术语/昵称/俗语）生成 → `logs/name_table_ja2zh.json`（90 条）；infobox name 字段偏全名且个别繁体，短名以译名表标准为准（菲莉丝/艾尔莎/罗姆爷）。昵称惯例按 infobox 严格区分：**ベア子=贝亚子、ベティ=贝蒂**（试点曾错误归一，已回改）。本地归一（translation fix 规则）仍保留作兜底（注入后 v2 仍漏了 1 处 贝阿特丽丝被兜底抓住）；`fix:translation` 的 generator 不覆盖 Module 命名空间是**有意为之**，确证时用 `-simulate` 手动指定子表。**若注入后质量仍不达标就换模型，不让另一模型全量检查**（かぐや様 类低频词处理不好时全检失去意义）
