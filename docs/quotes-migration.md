...
| 阶段 | 内容 | 产出 | 估时 | 依赖 |
|---|---|---|---|---|
| **P0 基础设施与试点** | LLM 接入脚本（OpenCode Zen 端点 + 译名表注入 + 429 退避）；Lua 生成器骨架（wikitext 转义、引号/花括号校验）；性能分解实测与架构方案（**主模块等价重写 + table= 参数已完成** 2026-08-09，见 modules.md；/all 拆动态子页的具体分法待 P2 测各表大小后定）；**用 OVA（17 条）走通全流程**：xlsx→对齐→补译→生成→上传→渲染验证 | 管线脚本 + OVA 上线 | 1 会话 | OpenCode key（已实测可用） |
