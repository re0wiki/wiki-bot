...
| **P2 动画实况解说** | xlsx 按日期→集数映射（注意 2016 深夜档次日偏移）；66 集 LLM 对齐 zh bullet↔ja tweet；无中文推文补译 ~2000 条；生成上传；渲染验证；**按播出批次拆子表**（OVA 已于 P0 上线；旧版/新编集版/第二季待建，建齐后处置原空占位页）；测各表展开大小 → 定 /all 动态子页拆法 | `鼠色猫语录/动画实况解说/*` 全量 | 3-4 会话 | P0 |
