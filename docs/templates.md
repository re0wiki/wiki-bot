...
- **`{{Documentation}}` 要并入已有 `<noinclude>` 内部**：追加第二个独立 `<noinclude>{{Documentation}}</noinclude>` 时，两个 noinclude 之间的换行会被 transclude 到引用页（多一个换行 = 段落分裂，精灵族文档首轮踩过）。推广（2026-08-03 修复实证）：**行内模板内容与其后 `<noinclude>` 之间也不能有换行**——换行会被 transclude，HTML 折叠为空格，行内使用出现多余间隙（`（{{Seirei}}）族` → `（精灵 ）族`）。当日修复 `Elf`/`Seirei`/`Yousei`/`加护`/`Copy`/`Tooltip`/`Ruby`/`Seirei or Elf`/`Yousei or Elf` 共 9 个模板（Tooltip 的换行经 Ringa 等二级泄漏）；块级模板（Clear/Main/Bot 等）尾部换行无害，不用管。
