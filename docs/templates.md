...
盘点脚本：`scripts/tools/template_inventory.py`（只读；输出到 `logs/template_inventory.json`）。
...
- 分类：顶层模板全部入树（2026-07-26 清零无分类模板）。
...
- **信息框不声明 previous/next 参数**（2026-08-22 用户裁决，含 bd——此前 bd 的「圆盘序列」字段曾为例外）：前后集/前后卷/前后篇导航全部由 `Tab/*` 承担。en 搬运带入的 `| previous/next =` 由 fix:para 常驻**删行**（保留同行的模板闭合 `}}`）。Tab 覆盖核验方法：zh 源码 `[[en:]]` 建 en→zh 映射（不用 langlinks 派生表）→ en prev/next 目标反查 zh 页 ∈ 该页 Tab 链接集；分层 Tab 两跳判定（本页 Tab 链接的季/章总页自带下一层 Tab）。
- **多语言书目数据用 per-语言参数家族**（仅 Infobox book，Module:Infobox book 渲染，语言表 12 种：ja/zh_hans/zh_hant/en/ko/pl/pt/fr/it/vi/ru/es）：`pages_*`/`date_*`/`isbn_*`。en 的单行堆积形态（`pages_ja = 292 (Japanese)<br>312 (Korean)…`）由 fix:para 常驻拆分——判据为每段都带已知语言括注（天然排除 `(Termination)`/`(BD)`/`(TV size)` 等同形异义与 game/bd/music 的非家族模板）、含 Japanese 段、同语言不重复（分册留人工），日期段顺带归一（`Month D, YYYY`→ISO、`Month YYYY`→`YYYY-MM`）；模板内已有目标参数时跳过该段保留人工值。表外语言（印尼语等）整参数保守跳过，扩展见 docs/todo.md。
...
- **`insource:` 全文搜索会分词漏配**（2026-08-02 实证）：对含 `:`/`-`/`."` 的词（如 `Gadget-Assert.css`）连 Common.css 自身都搜不到——排查 CSS/JS 引用链要靠 `scripts/tools/check_css_imports.py` 的定向核查而非全文搜索。
