...
- **鼠色猫语录空数据子模块**（帕克/福尔图娜）：空 `list` 占位是**刻意保留**（用户：以后可能补），勿删；Web连载网站上评论 已于 2026-08-12 启用（narou 评论 560 条全量）。（动画实况解说占位页已于 2026-08-10 随按批次拆表删除；abbr 机制已于 2026-08-11 废除——src 全量内联，30 表渲染逐字节验证等价。早期ask 于 2026-08-12 全量上线并拆为 2014-05~2014-08 / 2014-09~2015-10 两子表——渲染超 post-expand 2MB 墙，该上限是**整页聚合**而非每次调用独立，数据子表现 31 个。）
