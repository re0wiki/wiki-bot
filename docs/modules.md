...
2026-08-09 主模块（鼠色猫语录）查询函数等价重写：循环字符串拼接改 table.concat 缓冲（O(n²) 复制）、去掉每条目 3-4 次 mw.html.create 的纯开销（`<small>`/`<br/>` 改字面量）、2/3 语言下拉分支合并为 table.concat(rest, '<hr/>')；接口与输出 HTML 未变，7 组查询（全量/少命中/多命中/空结果/多关键词/日文 js/模板）parse 输出与旧版逐字节一致（候选页 Module:Sandbox/QuoteV2 对照，脚本 logs/p0_rewrite_validate.py）。性能基线：小查询 Lua CPU ~0.1s（limit report）；数据表 require+扫描 ~0.5s；耗时大头在输出侧 preprocess（全量 1.1MB 输出 ~3.4s → 新版 ~3.0s）——**数据 4 倍膨胀的瓶颈在输出侧与 /all 页，不在数据加载**。教训：Python 三引号字符串内嵌 Lua `'''`（粗体 wikitext）会被提前截断——生成 Lua 的 Python 脚本用 `r"""` 定界。
