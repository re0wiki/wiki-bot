...
- **Kana2Romaji 的 `メィ→mei`/`リィ→ri` 两条特判是必需项，勿删**（2026-07-30 考证）：它们服务 `角色:梅莉·波多尔德` 的 `name_ja_kanji = メィリィ·ポートルート`——メィリィ 是作者官方表记（なろう 6-46 节标题同款），小ぃ 不在规范拗音表内，删掉会以 `Meィriィ` 形式漏假名。输出 `Meiri` 与英文站信息框 Romaji 栏手写值一致（官方英文名是 Meili，栏位语义不同，不改）。历史：2021-03-06 建模块，次日（108839/108867）分两次针对该名补上这两条；初版表底子应抄自站外平文式表，后续フェ/フィ/フォ/ディ/ファ/ォ 等均是按与英文站手写罗马字的差异逐条打的补丁。2026-08-08 fix（logs/fix_kana2romaji_digraphs.py）：ウィ/ウェ/ウォ/トゥ/ドゥ 五条双字片假名原误置单音拍表——单音拍查找只按单字符进行，双字键永不可达，ウェ 被拆成 ウ(u)+ェ(漏假名)，`オットー·スーウェン` 错出 `Ottō Sūuェn`——已挪入双字音拍表。**教训：MONO_LIST 只收单字符假名，双字形式一律进 DI_LIST**。同日全角色罗马字审计（logs/audit_char_romaji.py / audit_char_romaji_3way.py，642 角色页三方比对 zh 假名 vs 模块输出 vs en 站 Kanji/Romaji 手写值）发现 **en 手写罗马字风格自身不统一**（长音 macron 与双写元音并存：Ottō vs Aachi/Hoorii），用户决策**不逐页跟随 en**：模块规则定为教科书平文式、zh 站内自洽（ん 在 b/p/m 前同化为 m、促音 chi→tch、长音一律 macron、无 van 小写等 en 特判；曾按 en 惯例改过一版又回退，见 git 历史）。配套清理（logs/plan_bc_edits.py / apply_bc_edits.py）：假名字段与 en Kanji 全量同步（・→· 归一，zh 别名/注记行保留，en 合并页不串行）；`name_ja_romaji` 手动值只保留给假名含汉字的页（土蜘蛛/大兔/白鯨/黑蛇 + 蒂亚斯的六世），其余 156 页手动值全清、由模块自动生成。审计局限：en 侧单行模板调用里的 `|Romaji =` 会被简单行正则误抓（菜月·昴 en 页 Romaji 实为空白）；en 合并角色页（萨尔姆/沃尔夫）的多行 Kanji 不能按行号对位。**2026-08-11 起彻底废弃手动填写**：Infobox character 罗马字行摘除 `source` 恒走本模块，9 个汉字名页面补 `name_ja_kana` 读音（字段语义：name_ja=日文名原文、kana=汉字名读音；同日 `name_ja_kanji` 改名 `name_ja`，见 templates.md），残留 `name_ja_romaji` 死行由 fix:para 末尾的作用域正则（不跨 `{{`/`}}`）自动删除，transferbot 新搬运页带入的 en 手写值同规则清除。其他 6 个信息框维持手动 romaji（整句标题需分词与助词/う段长音规则，评估后不自动化，见 templates.md）。
...
2026-08-09 主模块（鼠色猫语录）查询函数等价重写：循环字符串拼接改 table.concat 缓冲（O(n²) 复制）、去掉每条目 3-4 次 mw.html.create 的纯开销（`<small>`/`<br/>` 改字面量）、2/3 语言下拉分支合并为 table.concat(rest, '<hr/>')；接口与输出 HTML 未变，7 组查询（全量/少命中/多命中/空结果/多关键词/日文 js/模板）parse 输出与旧版逐字节一致（候选页 Module:Sandbox/QuoteV2 对照，脚本 logs/p0_rewrite_validate.py）。同日续：新增 `table=表名` 参数按名懒加载单表（同样逐字节验证后上线，logs/p0_table_param.py）。性能分解实测：小查询 Lua CPU ~0.1s；4x 数据模拟 require+扫描 CPU 0.4s——**Lua 侧非瓶颈；preprocess 只占 ~10% 且不可去**（去掉后 `<ref>` 变字面文本、模板不展开）；**硬约束是 post-expand include size 2MB**（4x 模拟全量输出被整体丢弃，`template omitted, post-expand include size too large`）。教训：Python 三引号字符串内嵌 Lua `'''`（粗体 wikitext）会被提前截断——生成 Lua 的 Python 脚本用 `r"""` 定界；difflib.SequenceMatcher 比较 MB 级字符串会卡死，大输出只比特征不比全文。
