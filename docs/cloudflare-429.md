...
- **惩罚窗口**：429 风暴刚过，全新进程（`retry_after=0`、无 throttle.ctrl）首发请求也会
  ...
  **2026-08-13 修正**：Retry-After 是**到固定解禁点的剩余时间**而非递增阶梯
  （13:02 给 3600、13:39 给 1345，均指向同一解禁点 ~14:02；期间零星探测不续罚）。
  实际封禁 ~40 分钟即解除，短于名义值。但**处罚期间单发探测会漏过**（匿名/裸 requests
  偶发 200），零星探测通过 ≠ 解禁——验证恢复要以真实任务（带会话 pywikibot）为准，
  或等到 Retry-After 指向的解禁点之后。
...
- 解除过程实测：封禁 ~40 分钟（13:02→~13:42）后各客户端（curl/匿名 python/带会话
  pywikibot）全恢复 200，短于名义 3600s。注意 13:12/13:20 曾有单发探测漏过（200），
  但 13:39/13:41 真实任务（pywikibot 带会话首发请求）仍被 429——**零星探测通过
  不代表解禁**，恢复判断要以真实任务或 Retry-After 解禁点为准。
