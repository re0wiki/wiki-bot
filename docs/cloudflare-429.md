...
- **惩罚窗口**：429 风暴刚过，全新进程（`retry_after=0`、无 throttle.ctrl）首发请求也会
  ...
  **2026-08-13 修正**：处罚**按 HTTP 方法分别执法——GET 全程放行、POST 封到固定解禁点**。
  Retry-After 是到固定解禁点的剩余时间（5 个采样 3600@13:02 / 1345@13:39 / 790@13:48 /
  663@13:50 / 407@13:55 全部精确指向 14:02:00），且期间十几次探测未移动解禁点
  （8-10 的「探测续罚」猜测证伪）。pywikibot 连读页面内容都走 POST（rvprop=content
  参数集大），bot 全部实际工作依赖 POST——**GET 探测通过对 bot 毫无意义，
  恢复验证必须测 POST**（最小 POST siteinfo 即可）或等到解禁点之后。
  **处罚记忆跨天保留**：距上次事故 3 天后，当天首个 429 直接给 3600 名义窗口（非初犯小值）。
...
- 复测（2026-07，探测脚本 `src/oneoff/probe_read_rate.py` / `probe_write_rate.py`）：
...
- 边界探测（2026-07，`src/oneoff/probe_read_boundary.py` / `probe_write_boundary.py`）：
...
- 封禁形态实测：**GET 全程放行、POST 封禁**（最小 POST siteinfo 也 429，同参数 GET 200）。
  pywikibot 读页面内容走 POST，故 bot 全功能被冻结。Retry-After 为固定解禁点
  （14:02:00）的剩余时间，期间多次 GET/POST 探测未移动解禁点。
  曾误判：13:12/13:20 仅凭 GET 探测 200 宣布「已解除」，实际 POST 仍封——
  **恢复验证必须测 POST，不能测 GET**。
...
- 实证（`src/oneoff/probe_async_concurrency.py`）：主线程全速预载 2000 页 +
...
裸 `requests` 的 BotPassword 登录完整流程见 `src/tools/verify_wiki_access.py`；
**login POST 也必须走带 429 退避的重试封装**，不能裸发。

## 验证

`src/tools/test_pwb_throttle.py`：100 次读 + 1 次沙盒写走 pywikibot，验证配置下零 429，
