...
- [ ] **S3 实况解说回收（零の幻影 bilibili 专栏）**：长月 S3 实况推已删（播后删是惯例，同 S1），公开存档层（wayback 快照多为 JS 空壳 / archive.today）无正文。零の幻影在 B 站专栏做日译中——爬取 B 站专栏回收（有工作量，2026-08-15 用户定暂不执行）。回收后入对应月表并补 zh
- [ ] **P8 不可恢复缺口（已核销，不再投入）**：2,021 条已删推（fxtwitter + 官方 oembed 双通道 404 实证；wayback 重试 650+/2021 零捞回——2019 后捕获全是 JS 空壳）。清单留档 `logs/p8_wb_pending.txt` + `~/p8_pending_tweets.md`。Twitter 无贴吧式吞帖，直接链接 404=作者删除
