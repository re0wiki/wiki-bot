...
主流程已收敛（见 `docs/modules.md` Kana2Romaji 条），剩下几类需要逐页人工判断：

- `角色:菜月菜穗子` / `角色:菜月贤一`：日文名是汉字（菜月 菜穂子 / 菜月 賢一），zh 假名字段一个填了读音 `なつき なおこ`、一个填了汉字本体，字段语义不统一；且无/缺 en 跨语言链接对照。要定字段语义（name_ja_kanji 填汉字名 + name_ja_kana 填读音）并补 `name_ja_romaji` 手动值。
- `角色:白羊` / `角色:黑狗`：汉字名角色（白羊/黒狗），假名字段空、罗马字空，原名罗马音（Shiro Hitsuji / Kuro Inu）只在 alias 里。是否补 `name_ja_kanji` + `name_ja_romaji` 待决策。
- `角色:希洛洛`：en 跨语言链接指向红链（en 无此页），无对照来源。
- `角色:梅里欧·阿嘎玛`：B 同步时按 en 把假名从 `メリオ·アガマ` 砍成 `メリオ`（en 只有名没有姓）——如果 zh 的アガマ 有官方出处，这属于 en 数据缺失而非 zh 错误，需要回退并考虑给 en 补数据。
