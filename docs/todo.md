...
- [ ] **英文字段清理**：2026-08-14 裁决——不新增 eq/ea/es；既有英文字段（月表内 eq/ea、十周年问答英文梗概）暂留待统一撤下。日文一手来源已收至上限（P6 回填 629 组 + P8 全量时间线，未覆盖部分实证不可恢复），条件已到顶，可排期
- [ ] **LLM 内容级去重**（P8 衍生）：既有条目未链推 id 但内容与 raw 推重复的（罗兹瓦尔表 ask 抄录那类）
- [ ] **全量中文翻译**（P8 衍生）：4,668 条 raw 推（含 Discord 回填 533）只有 ja，月表上增量补译
