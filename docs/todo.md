...
- [x] **narou 评论（计划 P4）——2026-08-12 完成**：560 条全量入库（占位表 Web连载网站上评论 启用）；笔名入 src、三栏带标注、不补章节锚点
- [x] **十周年问答（计划 P5）——2026-08-12 完成**：录音双引擎转写整合（whisper+MAI）→ 103 条 QA 全量上线；录音切 13 段传 File 空间，条目时间戳链对应分段
- [ ] **英文字段清理（待日文一手来源收齐后执行）**：2026-08-14 裁决——不再新增 eq/ea/es；既有英文字段（角色表 eq/ea、十周年问答英文梗概）暂留，待 P6 日文侧重核完成、日文一手来源全部到位后统一撤下
- [ ] **P8 全量推文时间线（计划已定于 quotes-migration.md P8 节）**：wayback 19,042 条唯一推全收 + 既有 34 表重排进 `YYYY-MM` 月表；id 级去重构建期做掉。执行后产生两个同级待办：**LLM 内容级去重**（未链推 id 但内容重复的既有条目）与**全量中文翻译**（月表上增量补）
- [ ] **S3 实况解说回收（零の幻影 bilibili 专栏）**：长月 S3 实况推已删（播后删是惯例，同 S1），公开存档层（wayback 快照多为 JS 空壳 / archive.today）无正文。**wiki Discord 转发频道已导出 3,663 条（2024-10~2025-10，含正文/embed，533 条已删推回填月表 ✅ 2026-08-15）**；频道外缺口走零の幻影 B 站专栏日译中爬取（有工作量，2026-08-15 用户定暂不执行）。回收后入对应月表并补 zh
- [ ] **P8 不可恢复缺口（已核销，不再投入）**：2,021 条已删推（fxtwitter + 官方 oembed 双通道 404 实证；wayback 重试 900+ 零捞回——2019 后捕获全是 JS 空壳；archive.today 用户自查无；**Discord 转发频道回填 533 条后余 ~1,490 条**）。清单留档 `logs/p8_wb_pending.txt` + `~/p8_pending_tweets.md` + `logs/p8_discord.json`。Twitter 无贴吧式吞帖，直接链接 404=作者删除
