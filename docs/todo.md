...
### 1. run_job 子进程用裸 `python`，可能跑错解释器

- 位置：`jobs/run_job.py:9`（`cmd = ["python", "pywikibot/pwb.py", *job]`）
- 问题：`python` 从 PATH 解析，不保证是项目 venv 的 3.14。PATH 上是其他 Python（如 3.11、无 opencc）时，translation fix 会在 `import opencc` 崩。AGENTS.md 推荐的 `PYTHONPATH= .venv/Scripts/python.exe main.py` 不会把 venv 放上 PATH，恰好会踩中。
- 修法：改用 `sys.executable`。

### 2. re0_image 无视 `-simulate`，干跑会真上传

- 位置：`scripts/re0_image.py`（全文无 `handle_args`）
- 问题：`config.simulate` 只在 `pwb.handle_args()` 里设置（pywikibot `bot.py:806`）。该脚本不解析参数，`python main.py 2 -s` 照样真实上传，与「`-s` 干跑验证」工作流矛盾。
- 修法：调用 `pwb.handle_args()`（至少消费 `-simulate`），或在 `main()` 开头显式检查 argv。
...
### 8. run_job 去掉 `shell=True`

- 位置：`jobs/run_job.py:26`
...
- 补 `run_job` 参数拼装（`-always`/`-auto -force`/transferbot 不加）的纯函数单测——配合待办 1 的修改做回归保护。
