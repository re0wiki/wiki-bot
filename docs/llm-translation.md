...
冷度键 = **最近一次非 IchiSanNi 编辑时间**（NiSanIchi 及其他账号均算人类）；页面由 IchiSanNi 创建且从无人类编辑的（transferbot 搬运页），取**创建时间**（内容新鲜度由搬运日决定，en 侧后续编辑由 revid 跟踪兜底）；**已处理页取处理时间**——处理即对齐了 en 最新人工改动，视为热页排队尾，避免反复追踪 en 微小更新而挤占从未翻过的远古条目。按冷度升序处理：活跃人类（如 2026 年中活跃的 Nekomeow151）正在碰的页自然排队尾，避免与人类编辑者撞车。管线编辑**不带 bot flag**（非需抑制通知的批量编辑），摘要统一为 `K3: revid <en_revid>（<时长>无人类编辑）`（由 done/summary 子命令机械生成，防止手写漂移；`K3翻译:` 为历史前缀，恢复解析兼容）——附冷度是为了让其他编辑者理解 bot 自动处理该页的原因。
