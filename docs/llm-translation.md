...
冷度键 = **最近一次非 IchiSanNi 编辑时间**（NiSanIchi 及其他账号均算人类）；页面由 IchiSanNi 创建且从无人类编辑的（transferbot 搬运页），取**创建时间**（内容新鲜度由搬运日决定，en 侧后续编辑由 revid 跟踪兜底）。按冷度升序处理：活跃人类（如 2026 年中活跃的 Nekomeow151）正在碰的页自然排队尾，避免与人类编辑者撞车。
