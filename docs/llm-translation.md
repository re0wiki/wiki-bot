...
- **done**：对 wiki 只读——以 prepare 时的 zh 现文与骨架为基线做事后机械核验（标记/页首/内链/模板/分类），通过即清场完成。脚本对 wiki 的唯一写入是 skip/auto-skip 的机械打标记。
...
每页的处理信息（冷度、en 源条目、预测编辑后的积压统计）由 stamp 生成的编辑摘要承载，随修订历史落账；skip 的摘要不含统计（沿用「同步标记：理由」格式）。
