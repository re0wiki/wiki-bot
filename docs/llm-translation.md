...
- 页尾：zh 现文的语言链接行保留即可（不增删、不改目标）；**语言链接与分类的相对顺序由循环任务的 cosmetic_changes 机械归一，agent 不需要刻意保持，done 也不核验**。
...
2. **框架不变量**：页首模板块（仅 To do 翻译类标注可按规则清理）以 prepare 时的 zh 现文为基线逐行比对（核验前先从新源码摘除标记行）。页尾语言链接与分类的相对顺序归 cosmetic_changes，不核验。
