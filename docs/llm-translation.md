...
- 页首：`{{Init}}`、`{{To do}}`、`{{Tab/...}}` 等行首模板块原样保留；裸 `{{To do}}` 改写为 `{{To do|由 K3 翻译自英文站，待校对润色}}`（`Template:To do` 的 `{{{1}}}` 会渲染在页首 indicator 里）。
...
- 翻译状态由 **wiki 编辑摘要**承载：摘要格式 `K3翻译: revid <en_revid>`（重跑时解析自身历史判断 en 是否已变化，无外部状态依赖）。
