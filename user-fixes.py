...
class _AltSub:
    ...
    def __call__(self, m):
        ...
        # 仅繁简差异的写法不改写：MediaWiki 简繁转换已能把繁体写法显示为标准名、
        # 把繁体链接解析到简体页，源码层归一毫无收益，只会误伤与繁体同形的日文
        # （聖域/神龍/加護/王選 等整类）。恒等命中也走此分支（只消耗文本保长匹配）。
        if t2s(m.group()) == target:
            return m.group()
...
user_fixes["translation"] = base | {
    "generator": generator_more,
    # 实质别名一律归一到官方简中标准名；仅繁简差异的写法由 _AltSub 判等不改写
    ...
    | {
        "inside": [
            ...
            # 信息框日文原名字段不归一
            r"(?m)^\s*\|\s*name_ja\s*=[^\n]*$",
            # 含假名的 CJK 连段 = 日文语境，整段不归一：兜住「实质别名出现在日文里」
            # 的情况（アナスタシアの最優紀行、王選前日譚 等书名/曲名/{{R}} 日文参数/
            # 日文引文；实测主空间命中全日文、零中文误挡）。仅繁简差异的写法已由
            # _AltSub 的 t2s 判等覆盖，本正则对它们是冗余保护。汉字区间只为扩展连段；
            # 顿号入集连通日文名内部枚举（水門都市残留組、プリステラ復興日誌 类）；
            # 。「」等句读/引号不进字符集，否则中日两段会被连成一段。
            # 盲区：纯汉字日文引用无假名可探测，仍靠 as-is 注释。
            r"[㐀-䶿一-鿿豈-﫿぀-ヿｦ-ﾝ々〆〇、]*[぀-ヿｦ-ﾝ][㐀-䶿一-鿿豈-﫿぀-ヿｦ-ﾝ々〆〇、]*",
        ],
    },
