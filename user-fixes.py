...
user_fixes["translation"] = base | {
    ...
    | {
        "inside": [
            ...
            # 信息框日文原名字段不归一
            r"(?m)^\s*\|\s*name_ja\s*=[^\n]*$",
            # 含假名的 CJK 连段 = 日文语境，整段不归一（卷/话/曲名、{{R}} 日文参数、
            # 日文引文等；实测主空间 137 处命中全日文、零中文误挡）。汉字区间只为
            # 扩展连段；顿号入集连通日文名内部枚举（水門都市残留組、プリステラ復興日誌
            # 类，实测新增保护全日文零误挡）；。「」等句读/引号不进字符集，否则中日
            # 两段会被连成一段。
            # 盲区：纯汉字日文引用（王選前日譚 类）无假名可探测，仍靠 as-is 注释。
